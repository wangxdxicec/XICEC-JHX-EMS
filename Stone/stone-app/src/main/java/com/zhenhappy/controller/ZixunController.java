...
import com.zhenhappy.dto.BaseRequest;
import com.zhenhappy.dto.ErrorCode;
import com.zhenhappy.dto.UrlResponse;
...
public class ZixunController extends BaseController {
	...
	@RequestMapping(value = "venueUrl", method = RequestMethod.POST)
	@ResponseBody
	public UrlResponse venueUrl(@RequestBody BaseRequest request) {
		LogUtil.logRequest(log, this, request);
		UrlResponse response = new UrlResponse();
		try {
			response.setUrl(systemConfig.getVal("cms_base_url") + getString(systemConfig.getVal("cms_base_url") + "/venue/789.htm"));
		} catch (Exception e) {
			log.error(e);
	...
	@RequestMapping(value = "serviceUrl", method = RequestMethod.POST)
	@ResponseBody
	public UrlResponse serviceUrl(@RequestBody BaseRequest request) {
		LogUtil.logRequest(log, this, request);
		UrlResponse response = new UrlResponse();
		try {
			response.setUrl(systemConfig.getVal("cms_base_url") + getString(systemConfig.getVal("cms_base_url") + "/venue/790.htm"));
		} catch (Exception e) {
			log.error(e);
			response.setErrorCode(ErrorCode.ERROR);
		}
