	...
	public LoginResponse login(@RequestBody LoginRequest request) {
		...
		try {
			...
			if (user != null && user.getIsCheck().intValue() == 1 && user.getIsDisable().intValue() == 0) {
				...
				response.setHeadUrl(systemConfig.getVal("head_base_url") + "cardHeadImg?cardId=" + user.getUserId());
	...
	public BaseResponse getPhoneMessageCheck(@RequestParam("phone") String phone, @RequestParam("token") String token,
		...
		try {
			...
			if (!m.matches()) {
				...
				//Boolean sendResult = ShortMsgSender.senderBySms100(phone, "感谢您注册中国厦门国际石材展手机客户端！验证码：" + sendCode + "请返回手机客户端进行验证。【厦门会展金泓信】", "http://www.sms100.cc/sms.aspx");
				Boolean sendResult = ShortMsgSender.senderBySms100(phone, "感谢您注册中国厦门国际石材展手机客户端！验证码：" + sendCode + "请返回手机客户端进行验证。【厦门会展金泓信】", url);
