...
public class ShortMsgSender {
    ...
    public static Boolean senderBySms100(String phoneNumber, String code, String url) {
        try {
            log.info(phoneNumber + " 请求短信验证：" + code);
            HttpClient httpClient = new DefaultHttpClient();
            String uri = url + "?mobile=" + phoneNumber + "&smsContent=" + URLEncoder.encode(code, "gb2312");
            HttpGet post = new HttpGet(uri);
//            HttpGet post = new HttpGet(url + "?mobile=" + phoneNumber + "&content=" + code + "&userid=776&password=12345678&account=xicecsms&action=send&sendTime=&extno=");
            HttpResponse response = httpClient.execute(post);
