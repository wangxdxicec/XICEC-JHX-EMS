...
<div id="tabs" class="easyui-tabs" data-options="fit:true,border:false,plain:true">
	<div title="短信申请列表" style="padding:5px">
		<table id="msgApply" data-options="url:'${base}/user/loadMsgApplyByPage',
         						   loadMsg: '数据加载中......',
...
<script>
	...
	$('#msgAllCustomers').click(function(){
		$.messager.confirm('确认删除','你确定要群发所有短信吗?',function(r){
			if (r){
				$.ajax({
	...
	$('#msgSelectedCustomers').click(function(){
		if(checkedItems.length > 0){
			$.messager.confirm('确认删除','你确定要群发所选申请客商短信吗?',function(r){
				if (r){
					$.ajax({
						url: "${base}/user/msgAllInlandStoneCustomers",
						type: "post",
	...
	function filter(){
		...
		$('#msgApply').datagrid('options').url = '${base}/user/queryMsgApplyByPage' + filterParm;
		$('#msgApply').datagrid('reload');
	}
	//----------------------------------------------------自定义函数结束--------------------------------------------------------//
	...
	$(document).ready(function () {
		...
		$('#msgApply').datagrid({
			onSelect:function (rowIndex, rowData){
				var row = $('#msgApply').datagrid('getSelections');
