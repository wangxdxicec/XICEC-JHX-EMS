...
<head>
    ...
    <script type="text/javascript" src="/manager/resource/jquery.min.js"></script>
    <script type="text/javascript" src="/manager/resource/easyui/jquery.easyui.min.js"></script>
    <script type="text/javascript" src="/manager/resource/common.js"></script>
    <script type="text/javascript" src="/manager/resource/easyui/easyui-validate.js"></script>-->
    <style>
        body {
            margin: 0px;
            padding: 0px;
...
<body>
<!-- 展团列表 -->
<table id="groups" data-options="url:'/manager/user/queryExhibitorGroupByPage',
         						   loadMsg: '数据加载中......',
		...
		$.messager.confirm('确认删除','你确定要删除展团吗?',function(r){
		    if (r){
		    	if(checkedItems.length > 0){
		    		$.ajax({
		                url: "/manager/user/deleteExhibitorGroups",
		                type: "post",
		                dataType: "json",
                    ...
                    handler: function () {
                    	if ($("#addExhibitorGroupForm").form("validate")) {
                        	if(document.addExhibitorGroupForm.groupName.value.trim() != "" || document.addExhibitorGroupForm.groupNameEn.value.trim() != ""){
                        		$.ajax({
                                    url: "/manager/user/addExhibitorGroup",
                                    type: "post",
                                    dataType: "json",
                    ...
                    handler: function () {
                    	if ($("#modifyExhibitorGroupForm").form("validate")) {
                        	if(document.modifyExhibitorGroupForm.groupName.value.trim() != "" || document.modifyExhibitorGroupForm.groupNameEn.value.trim() != ""){
                        		$.ajax({
                                    url: "/manager/user/modifyExhibitorGroup",
                                    type: "post",
                                    dataType: "json",
